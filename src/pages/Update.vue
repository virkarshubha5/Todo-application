<template>
  ...
  <div class="navbar-nav mr-auto">
      <li>
        <a @click="logOut" style="margin: 0 0 0 100%" class="btn btn-primary"
          >logout</a>
      </li>
    </div>
  </nav>
...
export default defineComponent({
  ...
  mounted() {
    ...
    const index: any = localStorage.getItem("index");
    console.log(index);
    const task = parsedUser.task;
    this.form.name = task[index].name;
    this.form.priority = task[index].priority;
    this.form.status = task[index].status;
  },
});
</script>
  <style>
